...
contract NFC is ERC721, ChainlinkClient, INFC {
  ...
  constructor(address _linkTokenAddress) public ERC721('Non-fungible Credit', 'NFC') {
    owner = msg.sender;
  ...
  function mintToken(bytes32 tokenHash) external override {
    require(tokenHash != 0x0, 'tokenHash cannot be blank');
  ...
  function getOriginalOwner(uint32 tokenId) public view override returns (address) {
  ...
  function isOriginalOwner(uint32 _tokenId) external view override returns (bool) {
    ...
    returns (bool)
  {
    ...
    require(_tickSet_tickEnds[_tickSet].length == 0, 'not empty');
    require(_tickEnds[0] > MIN_TICK && _tickEnds[_tickEnds.length - 1] <= MAX_TICK, 'bounds fail');
  ...
  function updateHolderTick(
    ...
    (address oracle, bytes32 jobID, uint256 fee) = reg.getScorerEndpoints(_scorerId);
    ...
    // );
    req.add('tokenId', typesLibrary.bytes32ToString(tokenId_hash[_tokenId]));
    ...
    recordChainlinkFulfillment(_requestId)
  {
    ...
    require(_tickResponse < _tickSet_tickEnds[response._tickSet].length, 'invalid tick returned');

    //need to check tickResponse is within # of ticks in tickset
    _tickSet_tokenId_scorer_tick[response._tickSet][response._tokenId][response._scorer] = tickData(
      uint32(_tickResponse),
      block.timestamp
    );
    emit ScoreUpdated(_tickResponse, block.timestamp, response._scorer, _tickResponse);
  }
  ...
  function payForUpdate(
    ...
    uint256 lastUpdatedTimeDifference = 1209600 -
      block.timestamp -
      _tickSet_tokenId_scorer_tick[_tickSet][_tokenId][_scorerId]._lastUpdated; //two weeks - time since last update
    if (lastUpdatedTimeDifference <= 0) {
      lastUpdatedTimeDifference = 0;
    }
    uint256 requiredAmount = mulDiv(1 * 10e15, lastUpdatedTimeDifference, 1209600); //if it's been more than last updated, then pay nothing. Otherwise, pay scaled fee.
    require(msg.value >= 1 * 10e15, 'update fee not sent'); //0.001 ETH
  ...
  function getTick(
    ...
    tickData memory ticks = _tickSet_tokenId_scorer_tick[_tickset][_tokenId][_scorer];
    tick = ticks._tick;
    lastUpdated = ticks._lastUpdated;
  }

  //get tokenID from token hash
  function getHashTokenId(bytes32 _tokenHash) external view override returns (uint32 tokenIDint) {
    return hash_tokenId[_tokenHash];
  }

  //get all ticks related to a certain scorer, add something later for get all scorers for a certain tickset
  function getMultipleTicksForSingleUser(
    ...
    uint256[] memory returnTicks = new uint256[](_scorer.length - 1);
    uint256[] memory returnTimes = new uint256[](_scorer.length - 1);
    for (uint256 i = 0; i < _scorer.length; i++) {
      returnTicks[i] = _tickSet_tokenId_scorer_tick[_tickset[i]][_tokenId][_scorer]._tick;
      returnTimes[i] = _tickSet_tokenId_scorer_tick[_tickset[i]][_tokenId][_scorer]._lastUpdated;
    }
    return (returnTicks, returnTimes);
  }

  //get the ticks for a specific tickset and scorer for a set of tokenIds
  function getMultipleUsersTicks(
    bytes32 _scorer,
    uint32[] memory _tokenId,
    uint256 _tickset
  ) external view override returns (uint256[] memory, uint256[] memory) {
    uint256[] memory returnTicks = new uint256[](_tokenId.length - 1);
    uint256[] memory returnTimes = new uint256[](_tokenId.length - 1);
    for (uint256 i = 0; i < _tokenId.length; i++) {
      returnTicks[i] = _tickSet_tokenId_scorer_tick[_tickset][_tokenId[i]][_scorer]._tick;
      returnTimes[i] = _tickSet_tokenId_scorer_tick[_tickset][_tokenId[i]][_scorer]._lastUpdated;
    }
    return (returnTicks, returnTimes);
  }
